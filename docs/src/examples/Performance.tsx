...
export function clampPerformanceControlValue(value: string | number): number {
  const parsedValue =
...
export function createPerformancePaths({
  ...
  const safeStrokeCount = clampPerformanceControlValue(strokeCount);
  const safePointsPerStroke = clampPerformanceControlValue(pointsPerStroke);
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <label className="grid gap-1 text-sm" htmlFor="performance-strokes">
            ...
            <input
              ...
              onChange={(event) =>
                setStrokeCount(
                  clampPerformanceControlValue(event.currentTarget.value),
                )
              }
