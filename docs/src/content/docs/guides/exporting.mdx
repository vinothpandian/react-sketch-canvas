  ...
  return (
    <>
      ...
      <button
        type="button"
        onClick={async () => {
          const dataUrl = await canvasRef.current?.exportImage("png"); // [!code highlight]
...
declare const canvasRef: React.RefObject<ReactSketchCanvasRef | null>;
// ---cut---
...
export function SaveLoad() {
  ...
  const save = async () => {
    const paths = (await canvasRef.current?.exportPaths()) ?? []; // [!code highlight]
  ...
  const load = () => {
    ...
    canvasRef.current?.loadPaths(paths); // [!code highlight]
  };
