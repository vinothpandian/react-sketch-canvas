...
function getOpenInLLMPrompt(url: string) {
	return `Read ${url} so you can answer questions about this page.`;
}
	...
	const [checked, onClick] = useCopyButton(async () => {
		...
		try {
			const promise = fetch(markdownUrl).then((res) => {
				if (!res.ok) {
					throw new Error(`Failed to fetch Markdown from ${markdownUrl}`);
				}
...
export function ViewOptionsPopover({
	...
	const items = useMemo(() => {
		...
		const q = getOpenInLLMPrompt(String(pageUrl));
